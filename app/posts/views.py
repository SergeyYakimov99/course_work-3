...
def posts_search():
    ...
    return render_template("search.html", query=query, posts=posts, number_posts=number_posts)
...
def posts_by_user(username):
    ...
    number_posts = len(posts)
    return render_template("user-feed.html", posts=posts, number_posts=number_posts)


@posts_blueprint.errorhandler(404)
def post_error(e):
    return "Такой пост отсутствует", 404